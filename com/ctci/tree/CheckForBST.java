...
public class CheckForBST {
	...
	boolean checkBST(){
		...
		return checkBST2(bst.getRoot(), Integer.MIN_VALUE, Integer.MAX_VALUE);
	}
	...
	boolean checkBST2(Node node, int min, int max){
		...
		if(node.getData() <= min || node.getData() > max)
			return false;
		return  checkBST2(node.getLeft(), min, node.getData()) && checkBST2(node.getRight(), node.getData(), max);
	}
